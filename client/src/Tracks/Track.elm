...
point info point =
    let
        ...
        color =
            info.primaryColor
    in
        Svg.svg [] [ cross (round x) (round y) 30 30 color ]
...
rectangleRegion info rectangle =
    let
        strX = toString rectangle.x
        strY = toString rectangle.y
        strWidth = toString rectangle.width
        strHeight = toString rectangle.height
    in
        Svg.svg
            [ SvgAt.x strX
            , SvgAt.y strY
