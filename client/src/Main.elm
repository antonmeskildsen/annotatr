port module Main exposing (Model, Msg(..), UpdateElement(..), createTrackInfoList, decodeTrack, deltaFrame, drawTrackEditingUI, drawTracks, frameToTime, getCurrentTime, getListIdx, getOrDefault, getVideoMeta, handleFeatureMove, init, inscribedCircleInfo, loadTrack, main, onTimeUpdate, pointInfo, rectangleRegionInfo, removeTrackIdx, replaceListIdx, saveTrack, setCurrentTime, setElement, startFrameUpdater, subscriptions, targetCurrentTime, timeToFrames, togglePlaying, trackInfo, update, updateTrackElement, updateTrackElementRange, videoMeta, view)

import Array
import Css exposing (absolute, height, left, pct, position, px, top, width)
import Debug
import Dom
...
update msg model =
    case msg of
        ...
        NewTrackFile ->
            let
                _ =
                    Debug.log "hej" model.trackLength
            in
        ...
        SetVideoMeta ( duration, width, height ) ->
            let
                _ =
                    Debug.log "frames:" (timeToFrames duration model.framesPerSecond)

                mtrack =
                    model.tracks

                len =
                    timeToFrames duration model.framesPerSecond
        ...
        AddTrack ->
            let
                _ =
                    Debug.log "type" model.newTrackType
                ...
                new =
                    { data = data, style = { primaryColor = "red", secondaryColor = "blue" }, name = model.newTrackName }
            in
            ( { model
                | tracks =
                    { length = model.tracks.length
                    , width = model.tracks.width
                    , height = model.tracks.height
                    , tracks = new :: model.tracks.tracks
                    }
              }
            , Cmd.none
            )
        ...
        CopyTrack idx ->
            let
                new =
                    List.drop idx model.tracks.tracks |> List.head
            in
            case new of
                Just a ->
                    ( { model
                    ...
                    case track.data of
                        PointTrack points ->
                            case Array.get model.currentFrame points of
                                Just p ->
                                    updateTrackElement model track model.currentFrame idx (PointElement { p | active = not p.active })

                                Nothing ->
                                    ( model, Debug.log "invalid frame index" Cmd.none )

                        InscribedCircleTrack circles ->
                            case Array.get model.currentFrame circles of
                                Just c ->
                                    updateTrackElement model track model.currentFrame idx (InscribedCircleElement { c | active = not c.active })

                                Nothing ->
                                    ( model, Debug.log "invalid frame index" Cmd.none )

                        RectangleRegionTrack rects ->
                            case Array.get model.currentFrame rects of
                                Just r ->
                                    updateTrackElement model track model.currentFrame idx (RectangleRegionElement { r | active = not r.active })

                                Nothing ->
                                    ( model, Debug.log "invalid frame index" Cmd.none )

                        _ ->
                            ( model, Debug.log "Empty track type" Cmd.none )

                Nothing ->
                    ( model, Debug.log "invalid track idx" Cmd.none )
...
updateTrackElement model track frame trackIdx element =
    let
        newTrackData =
            setElement frame element track.data

        newTrack =
            { track | data = newTrackData }

        newTrackList =
            replaceListIdx model.tracks.tracks trackIdx newTrack
...
updateTrackElementRange model track frame trackIdx element =
    let
        newTrackData =
            List.foldl (\i acc -> setElement i element acc) track.data (List.range frame model.trackLength)
...
handleFeatureMove model track idx event =
    let
                ...
                PointTrack points ->
                    case Array.get model.currentFrame points of
                        Just p ->
                            case event.keyCode of
                                Keyboard.Key.W ->
                                    setElement model.currentFrame ({ p | y = p.y - mult } |> PointElement) track.data

                                Keyboard.Key.A ->
                                    setElement model.currentFrame ({ p | x = p.x - mult } |> PointElement) track.data
        ...
        newTrackList =
            replaceListIdx model.tracks.tracks idx newTrack
...
trackActive : Track -> String
trackActive track =
    case track.data of
        PointTrack elem ->
            Array.filter (\e -> e.active) elem |> Array.length |> toString

        InscribedCircleTrack elem ->
            Array.filter (\e -> e.active) elem |> Array.length |> toString

        RectangleRegionTrack elem ->
            Array.filter (\e -> e.active) elem |> Array.length |> toString
    ...
    let
        infoElement idx track =
            ...
            div
                ...
                , div [ class "content" ]
                    [ div [ class "header" ] [ text (track.name ++ " : " ++ trackActive track) ]
...
pointInfo currentFrame idx p =
    let
        ...
        changeX xStr =
            UpdateTrackElement currentFrame idx ({ p | x = toFloatDefault xStr } |> PointElement)

        changeY yStr =
            UpdateTrackElement currentFrame idx ({ p | y = toFloatDefault yStr } |> PointElement)
    in
    div [ class "ui form", Json.map (HandleEditKeyboardEvent idx) decodeKeyboardEvent |> on "keydown" ]
        [ div [ class "fields" ]
            [ div [ class "field" ]
                [ label [] [ text "Active" ]
                , input [ class "ui checkbox", type_ "checkbox", onClick (ToggleTrackActive idx), checked p.active ] []
                ]
            , div [ class "field" ]
                [ label [] [ text "x" ]
            ...
            , button [ class "ui button", UpdateTrackElement (currentFrame + 1) idx (PointElement { x = x, y = y, active = True }) |> onClick ] [ text "Copy to next" ]
            , button [ class "ui button", CopyRest (currentFrame + 1) idx (PointElement { x = x, y = y, active = True }) |> onClick ] [ text "Copy to rest" ]
            ]
        ]


inscribedCircleInfo : Int -> Int -> InscribedCircle -> Html Msg
inscribedCircleInfo currentFrame idx ({ cx, cy, width, height, angle, active } as circ) =
    let
        toFloatDefault str =
            String.toFloat str |> Result.withDefault 0

        changeCx cxStr =
            UpdateTrackElement currentFrame idx ({ circ | cx = toFloatDefault cxStr } |> InscribedCircleElement)

        changeCy cyStr =
            UpdateTrackElement currentFrame idx ({ circ | cy = toFloatDefault cyStr } |> InscribedCircleElement)

        changeWidth widthStr =
            UpdateTrackElement currentFrame idx ({ circ | width = toFloatDefault widthStr } |> InscribedCircleElement)

        changeHeight heightStr =
            UpdateTrackElement currentFrame idx ({ circ | height = toFloatDefault heightStr } |> InscribedCircleElement)

        changeAngle angleStr =
            UpdateTrackElement currentFrame idx ({ circ | angle = toFloatDefault angleStr } |> InscribedCircleElement)
    in
    div [ class "ui form", Json.map (HandleEditKeyboardEvent idx) decodeKeyboardEvent |> on "keydown" ]
        [ div [ class "fields" ]
            [ div [ class "field" ]
                [ label [] [ text "Active" ]
                , input [ class "ui checkbox", type_ "checkbox", onClick (ToggleTrackActive idx), checked active ] []
                ]
            , div [ class "field" ]
                [ label [] [ text "Center X" ]
...
rectangleRegionInfo currentFrame idx ({ x, y, width, height, active } as rectangle) =
    let
        toFloatDefault str =
            String.toFloat str |> Result.withDefault 0

        changeX xStr =
            UpdateTrackElement currentFrame idx ({ rectangle | x = toFloatDefault xStr } |> RectangleRegionElement)

        changeY yStr =
            UpdateTrackElement currentFrame idx ({ rectangle | y = toFloatDefault yStr } |> RectangleRegionElement)

        changeWidth widthStr =
            UpdateTrackElement currentFrame idx ({ rectangle | width = toFloatDefault widthStr } |> RectangleRegionElement)

        changeHeight heightStr =
            UpdateTrackElement currentFrame idx ({ rectangle | height = toFloatDefault heightStr } |> RectangleRegionElement)
    in
    div
